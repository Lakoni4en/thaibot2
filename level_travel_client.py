...
class LevelTravelClient:
    ...
    async def search_tours(self) -> List[Tour]:
        ...
        # Aviasales принимает departure_at в формате YYYY-MM (весь месяц) или YYYY-MM-DD.
        departure_month = "2026-03"

        tours: List[Tour] = []

        params = {
            "origin": self._config.origin_city_code,
            "destination": self._config.destination_city_code,
        ...
        for f in flights:
            try:
                ...
                return_date_str = f.get("return_date")  # YYYY-MM-DD
                ...
                origin = f.get("origin", self._config.origin_city_code)
                destination = f.get("destination", self._config.destination_city_code)
